...
class VOCDataset(ImageDataset):
    ...
    def load_dataset(self):
        ...
        '''
        第一部获取文件列表中的index 系列
        第二部份获取文件名,并且确保文件的确存在，这个操作也是非常美丽的操作需要我们学习的地方共勉，宝贝们
        '''
    ...
    def _load_pascal_annotations(self):
        ...
        if os.path.exists(cache_file):
            ...
            return roidb
        #这一部分是我们处理我们的xml ，这里需要特别注意的是，我们为了节省每次处理的过程而将其存成了pkl文件
        gt_roidb = []
        for index in tqdm.tqdm(self.image_indexes,desc="load_data"):
            gt_roidb.append(self._annotation_from_index(index))
    ...
    def _annotation_from_index(self, index):
        ...
        #这个稀疏矩阵的取法为他的none zero，其中data 存放的都为1. 而none zero 存储的是其对应的坐标，来处理稀疏矩阵
